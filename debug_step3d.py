"""Debug step 3 - try data= instead of files= for step 3."""
...
# === Step 3: Try data= (URL-encoded) instead of files= (multipart) ===
...
print(f"\nStep 3 data keys: {list(data3.keys())}")

# Use data= (URL-encoded form) instead of files= (multipart)
resp4 = client.session.post(
    f"{client.BASE}{EDU_URL}",
    data=data3,
    headers=post_headers,
)
print(f"Step 3 (data=): status={resp4.status_code}")
...
has_campus = "not on campus" in resp4.text or "far_from_campus" in resp4.text
print(f"  has_error={has_error}, has_success={has_success}, has_campus={has_campus}")
...
body_soup = BeautifulSoup(resp4.text, "html.parser")
body_text = body_soup.get_text(strip=True)[:500] if body_soup.body else resp4.text[:500]
print(f"\nBody text:\n{body_text}")
